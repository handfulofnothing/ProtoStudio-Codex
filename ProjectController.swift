...
final class ProjectController: ObservableObject {
    ...
    private static func ensureTemplateFiles(at projectURL: URL, fileManager: FileManager) throws {
        ...
        let framerURL = projectURL.appendingPathComponent("framer")
        if !fileManager.fileExists(atPath: framerURL.path) {
            try fileManager.createDirectory(at: framerURL, withIntermediateDirectories: true)
        }

        try copyResourceIfNeeded(named: "framer", withExtension: "js", subdirectory: "framer", toDirectory: framerURL)
        try copyResourceIfNeeded(named: "coffee-script", withExtension: "js", subdirectory: "framer", toDirectory: framerURL)
    }

    private static func defaultResource(named name: String, withExtension ext: String) throws -> String {
        guard let url = ResourceBundle.bundle.url(forResource: name, withExtension: ext) else {
            throw NSError(domain: "ProjectController", code: 10, userInfo: [NSLocalizedDescriptionKey: "Missing template \(name).\(ext)"])
        }
        return try String(contentsOf: url)
    }

    private static func copyResourceIfNeeded(named name: String, withExtension ext: String, subdirectory: String? = nil, toDirectory directory: URL) throws {
        let destinationURL = directory.appendingPathComponent("\(name).\(ext)")
        if FileManager.default.fileExists(atPath: destinationURL.path) {
            return
        }

        guard let sourceURL = ResourceBundle.bundle.url(forResource: name, withExtension: ext, subdirectory: subdirectory) else {
            throw NSError(domain: "ProjectController", code: 11, userInfo: [NSLocalizedDescriptionKey: "Missing resource \(name).\(ext) in bundle"])
        }
